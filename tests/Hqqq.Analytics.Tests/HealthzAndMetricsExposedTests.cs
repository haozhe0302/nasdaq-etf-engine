...
namespace Hqqq.Analytics.Tests;

/// <summary>
/// Phase 2D1 — verifies that the worker-side management surface used by
/// hqqq-analytics exposes the standard Phase 2 endpoints. The analytics
/// jobs aren't booted; only the observability registration shared with
/// Program.cs.
/// </summary>
public class HealthzAndMetricsExposedTests
{
    [Fact]
    public async Task ManagementHost_ExposesHealthzAndMetrics_ForAnalyticsService()
    {
        ...
        hostBuilder.Services.AddHqqqObservability("hqqq-analytics", hostBuilder.Environment);
